...
import { useAddReference } from '../../api/referenceApi'

const AdminAddReference = () => {
    ...
    const onSubmit: SubmitHandler<any> = async (data, e: any) => {
        ...
        try {
            ...
            toast.success('Referans başarıyla eklendi', toastOptions)
            console.log(data) 
            navigate('/admin/referanslar')
        } catch (error) {
            toast.error('Referans eklenirken bir hata oluştu', toastOptions)
            console.log(error)
            console.log(data)
        }
    }
  ...
  return (
    <div className='p-12 w-full'>
         <h2 className="text-center text-white text-3xl mb-12 pb-2 border-b border-slate-600 font-semibold">Referans Ekle</h2>
         <form className="flex flex-col items-center" onSubmit={handleSubmit(onSubmit)}>
            <div className='flex flex-col gap-2 w-1/2'>
                <label htmlFor="img">Referans Resmi:</label>
                <input 
